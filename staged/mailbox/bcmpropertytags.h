...
typedef struct TPropertyTag		// header to be included in all tags below
{
	u32	nTagId;
	u32	nValueBufSize;			// bytes, multiple of 4		xzl: total val buf size
	u32	nValueLength;			// bytes			req:bit31 clear; resp: bit31 set, others are actual val lengh in bytes (from gpu)
