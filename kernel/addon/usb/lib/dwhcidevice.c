...
boolean DWHCIDeviceInitialize (TDWHCIDevice *pThis)
{
	...
	ConnectInterrupt (ARM_IRQ_USB, DWHCIDeviceInterruptHandler, pThis); // xzl: irq handler reg here...
	...
	// xzl: does rpi3 always have eth nic connected to usb? 
...
// xzl: blocking... request...  goes through multiple phases...
...
// xzl: only used by HCI devices. not massstorage
...
// xzl: init "usb core". config a bunch of regs..
boolean DWHCIDeviceInitCore (TDWHCIDevice *pThis)
{
	assert (pThis != 0);
...
void DWHCIDeviceEnableChannelInterrupt (TDWHCIDevice *pThis, unsigned nChannel)
{
	...
	uspi_EnterCritical ();		// xzl: this seems fine
...
// xzl: will spin waiting. since it actually uses irq + isr, it can be transformed
...
boolean DWHCIDeviceTransferStage (TDWHCIDevice *pThis, TUSBRequest *pURB, boolean bIn, boolean bStatusStage)
{
	...
	assert (pURB != 0);
	USBRequestSetCompletionRoutine (pURB, DWHCIDeviceCompletionRoutine, 0, pThis); // xzl: register callback
...
// xzl: start a trans stage then return
boolean DWHCIDeviceTransferStageAsync (TDWHCIDevice *pThis, TUSBRequest *pURB, boolean bIn, boolean bStatusStage)
{
	assert (pThis != 0);
	assert (pURB != 0);
	...
	else	// xzl: a split xfer. for data. now runs through its FSM
	{
...
void DWHCIDeviceStartChannel (TDWHCIDevice *pThis, TDWHCITransferStageData *pStageData)
{
	...
	// reset all pending channel interrupts		(xzl: for this channel)
	...
	uspi_CleanAndInvalidateDataCacheRange (
				PA2VA(DWHCITransferStageDataGetDMAAddress (pStageData)), 
...
void DWHCIDeviceChannelInterruptHandler (TDWHCIDevice *pThis, unsigned nChannel)
{
	...
	case StageSubStateWaitForChannelDisable:
		DWHCIDeviceStartChannel (pThis, pStageData);	//xzl:start a channel again
		return;

	case StageSubStateWaitForTransactionComplete: {
		uspi_CleanAndInvalidateDataCacheRange (	// xzl:use va to invalidate
			PA2VA(DWHCITransferStageDataGetDMAAddress (pStageData)), 
			DWHCITransferStageDataGetBytesToTransfer (pStageData));
		DataMemBarrier ();
	...
	case StageStateNoSplitTransfer:
		...
		USBRequestCallCompletionRoutine (pURB);		// xzl: no split... default to HID??
		break;
...
// xzl: irq handler... for entire device
void DWHCIDeviceInterruptHandler (void *pParam)
{
