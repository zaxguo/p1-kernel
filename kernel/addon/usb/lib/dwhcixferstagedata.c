...
//
// xzl: a simple wrapper around a xfer stage's data. (bookkeeping??)
...
// xzl: intent is to get PA.
u32 DWHCITransferStageDataGetDMAAddress (TDWHCITransferStageData *pThis)
{
	assert (pThis != 0);
	assert (pThis->m_pBufferPointer != 0);

	// return (u32) (uintptr) pThis->m_pBufferPointer;		// xzl: orig

	// debugging
	// u32 x = (u32) (uintptr) pThis->m_pBufferPointer;
	// LogWrite("usb", LOG_WARNING, "%x\n", x); 

	return VA2PA(pThis->m_pBufferPointer); 		// xzl add
}
