...
// xzl: meaing if inode was locked?
void
iputtest(char *s)
{
  if(mkdir("iputdir") < 0){
...
bigfile(char *s)
{
  ...
  enum { N = 200, SZ=8*1024}; // xzl: test enlarged files (doubly indirect ptr in inode)
  int fd, i, total, cc;
...
sbrkbasic(char *s)
{
  ...
  for(i = 0; i < 5000; i++){
    b = sbrk(1);
    if(b != a){     // xzl: a - old brk
      printf("%s: sbrk test failed %d %x %x\n", s, i, a, b);
      exit(1);
    }
    *b = 1;     // xzl: write to new brk
    a = b + 1;      
  }
  pid = fork();
  if(pid < 0){
...
stacktest(char *s)
{
#if 0  // xzl: TBD as we have on demand stack growth
  int pid;
  int xstatus;
