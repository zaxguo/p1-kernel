...
//
// xzl: unnecessary abstraction... 

#include <uspi/dwhciregister.h>
#include <uspios.h>
#include <uspi/assert.h>

// xzl: init a reg obj with phys addr
void DWHCIRegister (TDWHCIRegister *pThis, uintptr nAddress)
{
	assert (pThis != 0);
	pThis->m_bValid = FALSE;
	pThis->m_nAddress = (uintptr)PA2VA(nAddress);	// xzl:converted to kernel va...
}

// init: int a reg obj with phys addr & buffered init val (not written yet)
...
{
	assert (pThis != 0);
	pThis->m_nBuffer = *(volatile u32 *) pThis->m_nAddress;	// xzl: directly use this addr..
	pThis->m_bValid = TRUE;
...
void DWHCIRegisterSetAll (TDWHCIRegister *pThis)		// xzl: set all bits...
{
	assert (pThis != 0);
