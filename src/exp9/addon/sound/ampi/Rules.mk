...
ifeq ($(strip $(AARCH)),32)
	...
	ifeq ($(strip $(RASPPI)),3)
	ARCH	?= -DAARCH=64 -DAARCH64=1 -mcpu=cortex-a53 -mlittle-endian -mcmodel=small
	else ifeq ($(strip $(RASPPI)),4)
	ARCH	?= -DAARCH=64 -DAARCH64=1 -mcpu=cortex-a72 -mlittle-endian -mcmodel=small
	else
...
LDFLAGS	+= --section-start=.init=$(LOADADDR)	# xzl: only used for sample app
