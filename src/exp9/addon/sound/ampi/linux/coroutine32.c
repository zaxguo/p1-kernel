...
// xzl: circle implements its "linux emu" layer using cpp (e.g. simple kthread). 
// so the ampi redo here themselves w/ coroutine. intended to be a simple replacement
//  however, although ampi claimed to support aarch64, the asm here is aarch32.

//xzl: ref: https://github.com/kurocha/coroutine-arm64/blob/master/source/Coroutine/Context.s
...
{       // xzl: save arg (r2) as (r0) calling arg
