...
void enable_interrupt_controller()
{
    ...
    put32((unsigned long)PA2VA(TIMER_INT_CTRL_0), TIMER_INT_CTRL_0_VALUE);
    put32((unsigned long)PA2VA(ENABLE_IRQS_1), ENABLE_IRQS_1_USB | ENABLE_IRQS_1_AUX); 
    // put32((unsigned long)PA2VA(ENABLE_IRQS_1), ENABLE_IRQS_1_AUX); 
...
void handle_irq(void) {
    ...
    unsigned int irq = get32((unsigned long)PA2VA(INT_SOURCE_0)), irq0 = irq; 
    ...
    if (irq & GPU_SIDE_INTERRUPT) {
        unsigned int p1 = get32((unsigned long)PA2VA(IRQ_PENDING_1));
...
unknown:
    printf("Unknown pending irq: INT_SOURCE_0 %08x IRQ_BASIC_PENDING %08x" 
        ...
        irq0, 
        get32((unsigned long)PA2VA(IRQ_BASIC_PENDING)), 
        get32((unsigned long)PA2VA(IRQ_PENDING_1)),
        get32((unsigned long)PA2VA(IRQ_PENDING_2))
        );
    BUG(); 
}
#endif
