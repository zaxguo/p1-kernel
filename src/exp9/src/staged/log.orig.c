...
struct logheader {
  int n;
  int block[LOGSIZE];   // xzl: logged block#, in bcache
};
...
// current transaction commits.  xzl: write_head happens after write of data blocks
static void
write_head(void)
{
  struct buf *buf = bread(log.dev, log.start);
...
// xzl: write all dirty blocks from bcache; then write header (commit)
static void
commit()
{
  if (log.lh.n > 0) {
